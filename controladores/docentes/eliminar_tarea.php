...
$id = (int)($_POST['id'] ?? 0);
if(!$id){ http_response_code(422); echo json_encode(['ok'=>false]); exit; }

$docenteId = getDocenteId($conn);
$stmt = $conn->prepare("DELETE FROM tareas WHERE id=? AND docente_id=?");
$stmt->bind_param("ii", $id, $docenteId);
$ok = $stmt->execute();
$stmt->close();
