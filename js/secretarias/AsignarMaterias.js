document.addEventListener("DOMContentLoaded", () => {
  ...
  const url = "../../controladores/secretarias/controller_asignar_materia.php";
  ...
  // Clave = 4 letras materia + '-' + nombreGrupo (texto)
  ...
  formNuevo?.addEventListener("submit", async (e) => {
    ...
    if (r.status === "success") {
      ...
      Swal.fire({
        ...
        text: r.message || "Se guardó correctamente.",
        timer: 1400,
  ...
  formEditar?.addEventListener("submit", async (e) => {
    ...
    if (r.status === "success") {
      ...
      Swal.fire({
        icon: "success",
        title: "Actualizado",
        text: r.message || "Se actualizó correctamente.",
        timer: 1400,
        showConfirmButton: false,
      }).then(() => location.reload());
    } else {
  ...
  // ===== (SIN ELIMINAR) =====
  // Nota: No hay listener para .btn-eliminar porque Secretaría no puede eliminar.
  ...
  btnExportar?.addEventListener("click", function () {
    ...
    setTimeout(() => {
      // Ajusta esta ruta si tu exportador está en otra carpeta
      window.location.href =
        "../../controladores/secretarias/exportar_asignaciones.php?tipo=pdf";
