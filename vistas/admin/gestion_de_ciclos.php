...
// Obtener ciclos
$stmt = $pdo->query("SELECT * FROM ciclos_escolares ORDER BY id_ciclo ASC");
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <div class="search-bar">
                ...
                <input type="text" id="buscarCiclo" placeholder="Buscar..." />
            </div>
        ...
        <div class="main-content">
            ...
            <div class="table-card">
                <div class="card-title">
                    <h3><i class="fas fa-calendar-alt"></i> Ciclos Escolares</h3>
                </div>
                        ...
                        <tbody>
                            <?php if (!empty($ciclos)): ?>
                                <?php foreach ($ciclos as $row): ?>
                                    <tr data-id="<?= htmlspecialchars($row['id_ciclo']) ?>"
                                        data-nombre="<?= htmlspecialchars($row['nombre_ciclo']) ?>"
                                        data-inicio="<?= htmlspecialchars($row['fecha_inicio']) ?>"
                                        data-fin="<?= htmlspecialchars($row['fecha_fin']) ?>"
                                        data-activo="<?= htmlspecialchars($row['activo']) ?>">
                                        <td><?= htmlspecialchars($row['id_ciclo']) ?></td>
                                        <td><?= htmlspecialchars($row['nombre_ciclo']) ?></td>
                                        <td><?= htmlspecialchars($row['fecha_inicio'] ?: '-') ?></td>
                                        <td><?= htmlspecialchars($row['fecha_fin'] ?: '-') ?></td>
                                        <td>
                                            <button
                                                class="btn btn-outline btn-sm btn-toggle <?= $row['activo'] ? 'activo' : 'inactivo' ?>"
                                                data-id="<?= $row['id_ciclo'] ?>" data-activo="<?= $row['activo'] ?>">
        ...
        <div class="modal">
            ...
            <form id="formNuevo">
                <fieldset>
                    ...
                    <input type="text" name="nombre_ciclo" id="nombre_ciclo" required>
    ...
    <div class="modal-overlay" id="modalEditar">
        <div class="modal">
            ...
            <form id="formEditar">
                <fieldset>
                    <label for="editNombreCiclo">Nombre del Ciclo</label>
                    <input type="text" name="nombre_ciclo" id="editNombreCiclo" required>
    ...
    <script src="../../js/admin/CicloEscolar.js"></script>
</body>
